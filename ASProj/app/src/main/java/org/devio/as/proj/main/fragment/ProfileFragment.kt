...
import org.devio.`as`.proj.common.rn.HiRNBridge
...
//        item_collection.append(ITEM_PLACE_HOLDER + getString(R.string.item_collection))
        ...
        tab_item_history.setOnClickListener {
            goToBrowsing()
        }
        item_history.setOnClickListener { goToBrowsing() }
        queryLoginUserData()
        queryCourseNotice()
    }

    private fun goToBrowsing() {
        ARouter.getInstance().build("/rn/main").withString(HiRNActivity.HI_RN_BUNDLE, HiRNCacheManager.MODULE_NAME_BROWSING)
            .navigation(context)
    }
