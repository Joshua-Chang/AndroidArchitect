...
public final class Retrofit {
  ...
  public <T> T create(final Class<T> service) {
    ...
    return (T)
        Proxy.newProxyInstance(//newProxyInstance动态代理只能代理接口，所以都定义在接口里
            ...
            new InvocationHandler() {
              ...
              public @Nullable Object invoke(Object proxy, Method method, @Nullable Object[] args)
                  throws Throwable {
                //如果该方法是Object的方法，则请执行常规调用。
                if (method.getDeclaringClass() == Object.class) {
                  return method.invoke(this, args);
                }
                args = args != null ? args : emptyArgs;
                return platform.isDefaultMethod(method)//默认方法，java8后在接口方法前加default关键字，可以有实现
                    ? platform.invokeDefaultMethod(method, service, proxy, args)
                    : loadServiceMethod(method)/*1、加载请求方法，返回HttpServiceMethod*/.invoke(args);/*调用并传参*/
                /**
                 * @see HttpServiceMethod#invoke(Object[]) 首先创建Call对象,此时是OkHttpCall （requestFactory, args, callFactory, responseConverter作为参数传入）
                 * @see OkHttpCall 是Retrofit对Okhttp内的Call的包装
                 * tips:Retrofit只在OkHttpCall、RequestFactory中import了Okhttp。很好的隔离了Okhttp
                 *
                 * 然后内部调用{@link HttpServiceMethod#adapt(Call, Object[])}交给具体子类去转换
                 *
                 * @see HttpServiceMethod.CallAdapted#adapt(Call, Object[])
                 * @see HttpServiceMethod.SuspendForBody#adapt(Call, Object[])
                 * @see HttpServiceMethod.SuspendForResponse#adapt(Call, Object[])
                 */
              }
            });
  }
  ...
  ServiceMethod<?> loadServiceMethod(Method method) {
    ServiceMethod<?> result = serviceMethodCache.get(method);//缓存处理
    if (result != null) return result;

    synchronized (serviceMethodCache) {
      result = serviceMethodCache.get(method);
      if (result == null) {
        result = ServiceMethod.parseAnnotations(this, method);//2、解析请求方法的注解
  ...
  public static final class Builder {
    ...
    public Retrofit build() {
      ...
      if (callFactory == null) {//没有传callFactory，则默认为OkHttpClient 也是Call.Factory子类
        callFactory = new OkHttpClient();
      }

      Executor callbackExecutor = this.callbackExecutor;
      if (callbackExecutor == null) {//线程调度器，执行完请求后切回主线程
        // 在Android中defaultCallbackExecutor为MainThreadExecutor内部是handler（Platform-Android-defaultCallbackExecutor）
      ...
      /**
       * 添加不同的callAdapter，为不同的call转换不同的Callback回调
       * 虽然会添加默认callAdapter，但后续仍需处理，因为Retrofit2.8支持了携程，允许suspend标记方法：允许把返回值类型直接写成response体/Bean对象
       * @see ServiceMethod#parseAnnotations(Retrofit, Method) 确定callAdapter的类型
       */
