...
public class CategoryController {
    ...
    public ResponseEntity category(@RequestParam(value = "pageIndex", defaultValue = "1") @ApiParam("起始页码") int pageIndex
        ...
        return ResponseEntity.success(pageInfo.getList());
    }
    @ApiOperation(value = "商品子类别查询")
    @RequestMapping(value = "/subcategories/{categoryId}", method = RequestMethod.GET)
    public ResponseEntity subcategories(@ApiParam("子类别ID") @PathVariable String categoryId) {
        List<SubCategoryEntity> list = mService.getSubCategoryList(categoryId);
        return ResponseEntity.success(list);
    }
    @ApiOperation(value = "添加商品类别")
    @RequestMapping(value = "", method = RequestMethod.POST/*PUT修改某些信息才用*/)
