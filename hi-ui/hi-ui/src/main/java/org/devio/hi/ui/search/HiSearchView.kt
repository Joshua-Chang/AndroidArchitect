...
class HiSearchView @JvmOverloads constructor(
    ...
    /*init函数内需要的变量，要在init行前*/
    init {
        //初始化editText  --create-bind property --addview
        initEditText()
        //初始化右侧一键清除地小按钮   create-bind property --addview
        initClearIcon()
    ...
    fun setKeyword(keyword: String?, listener: OnClickListener) {
        ...
        kwClearIcon?.setOnClickListener {
            //1、点击了keyword-clear icon ,此时应该恢复默认提示语显示
    ...
    private fun initEditText() {
        ...
        editText?.setBackgroundColor(Color.TRANSPARENT)/*没有默认下划线*/
    ...
    private fun ensureKeywordContainer() {/*设置关键词后 搜索框的显示*/
        ...
        keywordTv?.maxLines = 1
        keywordTv?.ellipsize = TextUtils.TruncateAt.END
        keywordTv?.filters = arrayOf(InputFilter.LengthFilter(viewAttrs.keywordMaxLen))/*文本最大长度*/
    ...
    private fun toggleSearchViewsVisibility(showKeyword: Boolean) {
        ...
        changeVisibility(clearIcon, false)
        changeVisibility(searchIconHintContainer, !showKeyword)
